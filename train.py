...
def train(hyp):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # Recalculate Loss
            fgsm_loss, fgsm_loss_items = compute_loss(output, targets.to(device), model)

            # Backward
            if mixed_precision:
                with amp.scale_loss(fgsm_loss, optimizer) as fgsm_scaled_loss:
                    fgsm_scaled_loss.backward()
            else:
                fgsm_loss.backward()
